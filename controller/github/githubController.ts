...
import { getAuthUrl, handleCallback, getUserInfo } from "../../services/worldid/worldid";
...
export async function Callback(req: Request, res: Response) {
    ...
    try {
        const { idUser, userEmail } = await handleCallback(code as string, state as string);
        const url = `${WEB_URL}?id_user=${idUser}&email=${userEmail}`;
        return res.redirect(url);
    } catch (error: any) {
...
export async function GetUserInfo(req: Request, res: Response) {
    ...
    try {
        const userData = await getUserInfo(access_token as string, "Bearer");
