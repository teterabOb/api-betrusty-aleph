...
export async function handleCallback(code: string, state: string): Promise<{ idUser: string, email: string }> {
    ...
    if (githubInfoDBB.rowCount > 0) {
        await userDBB.updateTokenGithub(idUserString, access_token, expires_in.toString(), refresh_token, refresh_token_expires_in.toString(), email, jsonGithub);
    } else {
        await userDBB.saveTokens(idUserString, "did1", access_token, expires_in.toString(), refresh_token, refresh_token_expires_in.toString(), email, jsonGithub);
    }
